...
def verify_recur1(x, n, pnx, pnx1, pn1x1):
    if np.allclose(n * pnx, pnx1 * x - pn1x1):
        ...
        np.savetxt(
            "leg_data/leg02.dat",
            np.array(
                [x, np.ones(x.shape) * n, np.ones(x.shape) * (n - 1), pnx, pnx1, pn1x1],
                dtype=float,
            ).T,
            delimiter=",",
            fmt="%.12e",
...
def verify_recur2(x, n, pnx, pn1x, pn_1x):
    if np.allclose((2 * n + 1) * pnx * x, (n + 1) * pn1x + n * pn_1x):
        ...
        np.savetxt(
            "leg_data/leg03.dat",
            np.array(
                [x, np.ones(x.shape) * n, np.ones(x.shape) * (n - 1), pnx, pn_1x, pn1x],
                dtype=float,
            ).T,
            delimiter=",",
            fmt="%.12e",
...
if __name__ == "__main__":
    ...
    data1 = np.array(
        [
            x_data,
            leg(x=x_data, n=0, d=1),
            leg(x=x_data, n=1, d=1),
            leg(x=x_data, n=2, d=1),
            leg(x=x_data, n=3, d=1),
        ],
        dtype="double",
    )
